...
public class TradePersistentModelTest {
	...
	// double commisionAmt = tradeOrderFilledStatus.getFilledQuantity() *
	// 0.005d;
	//
	...
	// selectedRow);
	// }
	// org.trade.core.valuetype.Date openDate = (org.trade.core.valuetype.Date)
	// tradingdayModel
	// .getValueAt(tradingdayTable.convertRowIndexToModel(0), 0);
	// org.trade.core.valuetype.Date closeDate = (org.trade.core.valuetype.Date)
	// tradingdayModel
	...
	public void testReassignStrategy() {
		...
		try {
			...
			if (toStrategy.equals(newStrategy)) {
				_log.error("testReassignStrategy: equals toStrategy Id: "
						+ toStrategy);
				_log.error("testReassignStrategy: equals toStrategy Id: "
						+ newStrategy);
				assertEquals(
						"testReassignStrategy: Strategy should now be equal",
						toStrategy, newStrategy);
			} else {
				_log.error("testReassignStrategy: not equals toStrategy Id: "
						+ toStrategy.getIdStrategy());
				_log.error("testReassignStrategy: not equals toStrategy Id: "
						+ newStrategy.getIdStrategy());
				assertEquals(
						"testReassignStrategy: Strategy should now be equal",
						toStrategy.getIdStrategy(), newStrategy.getIdStrategy());
			}
